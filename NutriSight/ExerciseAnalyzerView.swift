...
struct ExerciseAnalyzerView: View {
    ...
    private func readExercisesAloud() {
        let introduction = "Here are the exercise options you can choose from: Squat, Plank, Push-up, Lunge, Glute Bridge, Calf Raise, Wall Sit, Shoulder Press, Tricep Dip, Bicycle Crunch, Superman, Mountain Climber, Jumping Jack, Burpee, High Knees, Box Jump, Kettlebell Swing, Russian Twist, Step-up"
        speechSynthesizer.speak(introduction) { [self] in
            self.speakExerciseGroups() // Start speaking exercises after the introduction
        }
    }

    private func speakExerciseGroups(groupIndex: Int = 0) {
        let exerciseGroups = exercises.chunked(into: 5) // Split exercises into groups of 5

        guard groupIndex < exerciseGroups.count else {
            // All groups have been spoken, provide final instructions
            speechSynthesizer.speak("Now, click the button to say the exercise you want to perform.") {}
            return
        }

        let group = exerciseGroups[groupIndex]
        let groupText = group.joined(separator: ", ") // Combine exercises in the group into a single string

        // Speak the current group of exercises
        speechSynthesizer.speak(groupText) { [self] in
...
struct FeedbackView: View {
    ...
    private func speakFeedback() {
        print("speakFeedback() called")
        guard !feedbackSummary.isEmpty else {
            print("No feedback to speak.")
            return
        }
        print("Speaking feedback: \(feedbackSummary)")
        speechSynthesizer.speak("Here's your feedback: \(feedbackSummary)") {
            print("Speech completion handler called")
        }
    }
...
class SpeechSynthesizer: NSObject, ObservableObject, AVSpeechSynthesizerDelegate {
    ...
    func speak(_ text: String, completion: @escaping () -> Void) {
        ...
        // Set a timer to check if speech has started
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            if !self.synthesizer.isSpeaking {
                print("Speech did not start after 1 second")
            }
        }
    }
